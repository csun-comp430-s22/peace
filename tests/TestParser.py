...
class TestParser(unittest.TestCase):
    ...
    def test_match(self):
        ...
        tree_expected = "(statement (match_stmt match (expression bar) { (match_case (match_pattern x) => (expression (expression y) = (expression x))) , (match_case (match_pattern 2) => (expression (expression y) = (expression x))) } ;))"
        parser = create_parser_for(test_input)
        tree = parser.statement()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_case_(self):
        ...
        tree_expected = "(match_stmt match (expression x) { (match_case (match_pattern 4) => (expression (expression y) = (expression 4))) })"
        parser = create_parser_for(test_input)
        tree = parser.match_stmt()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)

    def test_case_multiple(self):
        test_input = "match x {1 => y = 4, 2 => y = 5, 3 => y = 6}" # case: 4 matcharrow exp
        tree_expected = "(match_stmt match (expression x) { (match_case (match_pattern 1) => (expression (expression y) = (expression 4))) , (match_case (match_pattern 2) => (expression (expression y) = (expression 5))) , (match_case (match_pattern 3) => (expression (expression y) = (expression 6))) })"
        parser = create_parser_for(test_input)
        tree = parser.match_stmt()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_pattern(self):
        ...
        tree_expected = "(statement (match_stmt match (expression toMatch) { (match_case (match_pattern pie) => (expression (expression toMatch) = (expression pie))) , (match_case (match_pattern 4) => (expression (expression toMatch) = (expression 4))) }))"
        parser = create_parser_for(test_input)
        tree = parser.statement()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_parameter(self):
        ...
        tree_expected = "(func_stmt (atype (basetype bool)) someFunc ( (parameter indicator : (atype (basetype bool))) ) { })"
        parser = create_parser_for(test_input)
    ...
    def test_parameter_multiple(self):
        ...
        tree_expected = "(func_stmt (atype (basetype void)) person ( (parameter name : (atype (basetype string))) , (parameter age : (atype (basetype int))) ) { })"
        parser = create_parser_for(test_input)
        tree = parser.func_stmt()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_func_stmt(self):
        ...
        tree_expected = "(func_stmt (atype (basetype int)) addition ( (parameter num1 : (atype (basetype int))) , (parameter num2 : (atype (basetype int))) ) { (statement (vardec let sum : (atype (basetype int)) = (expression (expression num1) + (expression num2))) ;) })"
        parser = create_parser_for(test_input)
        tree = parser.func_stmt()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_enumdef(self):
        ...
        tree_expected = "(enumdef enum (expression cars) = { (cdef ford : (atype (basetype string)) ;) })"
        parser = create_parser_for(test_input)
        tree = parser.enumdef()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_enumdef_multiple_cdef(self):
        ...
        tree_expected = "(enumdef enum (expression colors) = { (cdef blue : (atype (basetype int)) ;) (cdef green : (atype (basetype int)) ;) (cdef violet : (atype (basetype int)) ;) })"
        parser = create_parser_for(test_input)
        tree = parser.enumdef()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
    ...
    def test_program(self):
        ...
        tree_expected = "(program (enumdef enum (expression nums) = { (cdef one : (atype (basetype int)) ;) }) (func_stmt (atype (basetype void)) main ( ) { (statement (print_stmt print ( (expression ok) ) ;)) }))"
        parser = create_parser_for(test_input)
        tree = parser.program()
        tree_actual = Trees.toStringTree(tree, None, PeaceParser)
        assert(tree_expected == tree_actual)
